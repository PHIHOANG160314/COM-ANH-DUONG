#!/usr/bin/env python3
"""
Menu Sync Script v2 - Sync 58 menu items from Excel to Supabase
Handles category_id mapping instead of category text
"""
...
# Category name to ID mapping (Vietnamese names from Excel to DB category_id)
CATEGORY_MAPPING = {
    "CÁ": "rice",      # Map to Cơm (rice dishes)
    "THỊT": "rice",
    "CANH": "rice",
    "VỊT+GÀ": "rice",
    "KHÁC": "rice",
...
def load_excel_menu(filepath: str) -> List[Dict]:
    ...
    for _, row in df.iterrows():
        ...
        item = {
            ...
            "category_id": CATEGORY_MAPPING.get(category_text, "rice"),  # Default to rice
            "category_text": category_text,  # Keep for reference
            "stt": int(row["STT"])
        }
    ...
    for excel_item in excel_items:
        ...
        if name not in db_lookup:
            ...
            if (db_item["price"] != excel_item["price"] or
                db_item.get("category_id") != excel_item["category_id"]):
                # Need update
    ...
    if to_insert:
        ...
        for item in to_insert:
            try:
                ...
                print(f"   ✅ {item['name']} - {item['price']:,}đ ({item['category_text']})")
...
def main():
    """Main execution flow"""
    print("=" * 60)
    print("🍽️  MENU SYNC SCRIPT v2 - Cơm Ánh Dương")
    print("=" * 60)

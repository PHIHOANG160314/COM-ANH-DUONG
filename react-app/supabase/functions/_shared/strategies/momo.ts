...
export class MoMoStrategy implements IPaymentStrategy {
  ...
  constructor() {
    ...
    this.endpoint = Deno.env.get("MOMO_ENDPOINT") ?? "https://test-payment.momo.vn/v2/gateway/api/create";
    ...
    this.ipnUrl = Deno.env.get("MOMO_IPN_URL") ?? `https://${projectRef}.supabase.co/functions/v1/handle-webhook?provider=momo`;
  ...
  async createPaymentUrl(request: PaymentRequest): Promise<PaymentResponse> {
    ...
    try {
        ...
        if (data.resultCode === 0 || data.resultCode === 9000) { // 9000 is for pending/auth
             return {
